...
Probabilistic::Probabilistic(const std::string &filename, const bool directed,
                             const double verb)
    : Graph(filename, directed), verbose(verb) {
...
bool Probabilistic::compute_finished(Status *status) const {
	...
	for (i = 0; i < status->k - 1; i++) {
		...
		err_l[i] =
		    compute_f(bet[i], status->n_pairs, delta_l_guess[status->top_k[i]]);
		err_u[i] =
		    compute_g(bet[i], status->n_pairs, delta_u_guess[status->top_k[i]]);
	}
	bet[i] = status->approx_top_k[i] / status->n_pairs;
	...
	if (absolute) {
		...
		for (uint32_t i = 0; i < status->k; i++) {
			if (i == 0) {
				...
				status->finished[i] = (bet[i - 1] - err_l[i - 1] > bet[i] + err_u[i]) &&
...
double Probabilistic::compute_f(const double btilde, const uint64_t iter_num,
                                const double delta_l) const {
	double tmp = (((double)omega) / iter_num - 1. / 3);
	double err_chern =
	    (log(1. / delta_l)) * 1. / iter_num *
	    (-tmp + sqrt(tmp * tmp + 2 * btilde * omega / (log(1. / delta_l))));
	return min(err_chern, btilde);
}

// Computes the function g that bounds the betweenness of a vertex from above.
// For more information, see Borassi, Natale (2016).
double Probabilistic::compute_g(const double btilde, const uint64_t iter_num,
                                const double delta_u) const {
	double tmp = (((double)omega) / iter_num + 1. / 3);
	double err_chern =
	    (log(1. / delta_u)) * 1. / iter_num *
	    (tmp + sqrt(tmp * tmp + 2 * btilde * omega / (log(1. / delta_u))));
...
void Probabilistic::print_status(Status *status, const bool full) const {
	if (full) {
		std::cout << std::setprecision(6) << endl
		          << "Finished after " << status->n_pairs << " iterations." << endl;
	} else {
		std::cout << std::setprecision(6) << endl
		          << "Situation after " << status->n_pairs << " iterations."
		          << endl;
	}
	...
	std::cout << "Time compute finished: "
	          << time_comp_finished[omp_get_thread_num()] << endl;
	...
	if (absolute) {
		...
		for (uint32_t i = 0; i < status->k; i++) {
			...
			max_interval =
			    max(max_interval, compute_f(status->approx_top_k[i] / status->n_pairs,
			                                status->n_pairs, delta_l_guess[v]));
			max_interval =
			    max(max_interval, compute_g(status->approx_top_k[i] / status->n_pairs,
			                                status->n_pairs, delta_u_guess[v]));
		}
		...
		for (i = 0; i < k; i++) {
			...
			cout << std::setw(8) << status->top_k[i] << " "
			     << bet - compute_f(bet, status->n_pairs,
			                        delta_l_guess[status->top_k[i]])
			     << " ";
			cout << bet << " "
			     << bet + compute_g(bet, status->n_pairs,
			                        delta_u_guess[status->top_k[i]])
			     << endl;
		}
		if (full) {
			double betk = status->approx_top_k[k - 1] / status->n_pairs;
			double lbetk = betk - compute_f(betk, status->n_pairs,
			                                delta_l_guess[status->top_k[k - 1]]);
			uint32_t pos = k + 1;
			for (i = k; i < status->k; i++) {
				double bet = status->approx_top_k[i] / status->n_pairs;
				if (bet + compute_g(bet, status->n_pairs,
				                    delta_u_guess[status->top_k[i]]) >
				    lbetk) {
			...
			for (i = k; i < status->k; i++) {
				double bet = status->approx_top_k[i] / status->n_pairs;
				max_upper =
				    max(max_upper, bet + compute_g(bet, status->n_pairs,
				                                   delta_u_guess[status->top_k[i]]));
			}
			double bet = status->approx_top_k[status->k - 1] / status->n_pairs;
			max_upper = max(max_upper,
			                bet + compute_g(bet, status->n_pairs, delta_u_min_guess));

			cout << std::setw(8) << "Others"
			     << ") <" << max_upper;
		}
	}
	cout << endl;
...
// Sample one shortest path and updates the ranking of the betweenness
// approximations.
...
void Probabilistic::compute_bet_err(Status *status, double *bet, double *err_l,
                                    double *err_u) const {
...
// The computation is based on the heuristic in the paper Borassi, Natale
// (2016).
void Probabilistic::compute_delta_guess() {
	double balancing_factor = 0.001;
	double a = 0,
	       b = 1. / err / err *
	           log(get_nn() * 4 * (1 - balancing_factor) / delta),
	       c = (a + b) / 2;
	double sum;
	...
	while (b - a > err / 10) {
		...
		sum += exp(-c * err_l[union_sample - 1] * err_l[union_sample - 1] /
		           bet[union_sample - 1]) *
		       (get_nn() - union_sample);
		sum += exp(-c * err_u[union_sample - 1] * err_u[union_sample - 1] /
		           bet[union_sample - 1]) *
		       (get_nn() - union_sample);
	...
	delta_l_min_guess = exp(-b * err_l[union_sample - 1] *
	                        err_l[union_sample - 1] / bet[union_sample - 1]) +
	                    delta * balancing_factor / 4. / get_nn();
	;
	delta_u_min_guess = exp(-b * err_u[union_sample - 1] *
	                        err_u[union_sample - 1] / bet[union_sample - 1]) +
	                    delta * balancing_factor / 4. / get_nn();
	;
	...
	for (uint32_t i = 0; i < union_sample; i++) {
		uint32_t v = status.top_k[i];
		delta_l_guess[v] = exp(-b * err_l[i] * err_l[i] / bet[i]) +
		                   delta * balancing_factor / 4. / get_nn();
		;
		delta_u_guess[v] = exp(-b * err_u[i] * err_u[i] / bet[i]) +
		                   delta * balancing_factor / 4. / get_nn();
		;
	}
	free(bet);
...
// Runs the algorithm.
// INPUT: k is the number of betweenness that have to be approximated (if k=0
// all betweenness are approximated with absolute error); delta is the
// probabilistic guarantee; err is the maximum error allowed; union_sample and
// start_factor are parameters of the algorithm that are automatically chosen.
void Probabilistic::run(uint32_t k, double delta, double err,
                        uint32_t union_sample, uint32_t start_factor) {
	this->absolute = (k == 0);
	this->err = err;
	this->delta = delta;
	this->start_factor = start_factor;
	this->omega =
	    0.5 / err / err * (log2(estimate_diameter() - 1) + 1 + log(0.5 / delta));
	...
	uint32_t *random_seed =
	    (uint32_t *)malloc(omp_get_max_threads() * sizeof(uint32_t));
